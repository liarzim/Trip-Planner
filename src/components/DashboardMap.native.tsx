...
export default function DashboardMap({ events, focusedEventId }: DashboardMapProps) {
    ...
    if (geoEvents.length > 0 && mapRef.current) {
      setTimeout(() => {
        ...
        geoEvents.forEach((e) => {
          ...
          if (e.type.toLowerCase() === 'flight') {
            const origLat = typeof e.originLatitude === 'number' ? e.originLatitude : 32.0094;
            const origLon = typeof e.originLongitude === 'number' ? e.originLongitude : 34.8769;
            allCoords.push({ latitude: origLat, longitude: origLon });
          }
        });
  ...
  const driveEvents = geoEvents.filter((e) => e.type.toLowerCase() !== 'flight');
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {geoEvents.map((item) => {
          if (item.type.toLowerCase() === 'flight') {
            ...
            const start = { latitude: origLat, longitude: origLon };
